...
//	int day_missing <- true_date.day;
	...
	//OSM file to load
	file<geometry> osmfile <-  file<geometry>(osm_file("../includes/maps/emp.osm", filtering))  ;
	...
	int nb_people <- 30; //number of people in the simulation
	...
	float max_driving_speed <- 30 #km / #h;
	...
	point Break_Point_Resto <- nil;
	point Break_Point_Coffee <- nil;
	...
	float demographic_driving <- 18.6;
	...
	init {
		...
		create people number: nb_people {
			...
			dist_helper <- gamma_rnd(21.26,16.03);
			start_work <- int(dist_helper +385.24); //in minutes
			start_work_hour <- start_work div 60;
			start_work_min <- start_work mod 60;
			dist_helper <- gamma_rnd(5.5,31.33);
			end_work <- dist_helper + 954.66;
			end_work_hour <- end_work div 60;
			end_work_min <- end_work mod 60;
			
			break_start_hour <- start_work_hour + ((end_work_hour - start_work_hour) div 2);
			break_start_min <- 0;
			
			break_end_hour <- break_start_hour;
			break_end_min <- rnd(15,59);
...
//				}
			...
			ask building.population {
				if (name = "building39") {
					Break_Point_Resto <- location;
					//write Break_Point;
					self.color <- #purple;
					}
				if (name = "building53") {
					Break_Point_Coffee <- location;
					self.color <- #orange;
					}
				}		
...
//				}
	...
	reflex missing_person_nearby when: agents_at_distance(0.007#km) contains_any missing_person {
		...
		if(walking_bool){
			...
			write "Walking " + self + "and near missing child";
			...
			close_call<-close_call+1;
			write ("Driving " + self + "and near missing child");
		...
		else if (objective != "resting"){  //possibility to find the missing child only when working or doing a break or a leisure activity, not when at home
			close_call<-close_call+1;
...
experiment Batch_Optimization_No_Times_Found type: batch repeat: 100 keep_seed: true until: ( (time / #day) > 2) {
	parameter "Number of People in Area" var: nb_people min:5 max:1000 step: 5;
    ...
    reflex save_results_explo {
        ask simulations {
            save [int(self),nb_people, self.times_found, self.times_found_walking, self.times_found_driving, self.times_found_resting] 
                   to: "../results_no_times_NTUA_repeat100_5to1000_step5_simstep1mn.csv" type: "csv" rewrite: (int(self) = 0) ? true : false header: true;
        }        
    }
}

experiment Batch_Optimization_First_Time type:batch repeat: 10 keep_seed: true until: ( times_found >= 1 or days_that_is_missing > 40) {
	parameter "Number of People in Area" var: nb_people min:5 max:1000 step: 5;
    ...
    reflex save_results_explo {
        ask simulations {
            save [int(self),nb_people, (self.time / #day), (self.time / #minute) ] 
                   to: "../results_first_time_mp_NTUA_repeat10_5to1000_step5_simstep1mn.csv" type: "csv" rewrite: (int(self) = 0) ? true : false header: true;
        }        
    }
}